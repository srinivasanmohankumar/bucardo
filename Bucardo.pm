        ...
        for my $pk_values (@pkvals) {
            ...
            ##Initializing the srccmd
                ...
                ## Two Level logic 
                    ...
                    $srccmd = sprintf '%sCOPY (%s FROM ONLY %s t1 INNER JOIN public.crm_emails t2 ON t1.crm_email_id = t2.id WHERE t2.project_id=1597 %s) TO STDOUT%s',
                    $self->{sqlprefix},
                    $SELECT,
                    $source_tablename,
                    $mode eq 'fullcopy' ? '' : " AND t1.$Table->{pklist} = ANY(?)",
                    ...
                    $srccmd = sprintf '%sCOPY (%s FROM ONLY %s t1 INNER JOIN public.charges t2 ON t1.charge_id = t2.id WHERE t2.project_id=1597 %s) TO STDOUT%s',
                    $self->{sqlprefix},
                    $SELECT,
                    $source_tablename,
                    $mode eq 'fullcopy' ? '' : " AND t1.$Table->{pklist} = ANY(?)",
