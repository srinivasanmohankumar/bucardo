        ...
        for my $pk_values (@pkvals) {
            ...
            ##Initializing the srccmd
                ...
                if(exists $hash1{$source_tablename}) {
                    $srccmd = sprintf '%sCOPY (%s FROM ONLY %s t1 WHERE t1.project_id=2 %s) TO STDOUT%s',
                    $self->{sqlprefix},
                    $SELECT,
                    $source_tablename,
                ...
                ##Two Level joins initial copy logics
                    ...
                    $srccmd = sprintf '%sCOPY (%s FROM ONLY %s t1 INNER JOIN public.users_projects t2 ON t1.user_id = t2.user_id WHERE t2.project_id=2 %s) TO STDOUT%s',
                    $self->{sqlprefix},
                    $SELECT,
                    $source_tablename,
                    $mode eq 'fullcopy' ? '' : " AND t1.$Table->{pklist} = ANY(?)",
                ...
                elsif($source_tablename eq 'shared.deals_people') {
                    $srccmd = sprintf '%sCOPY (%s FROM ONLY %s t1 INNER JOIN shared.deals t2 ON t1.deal_id = t2.id WHERE t2.project_id=2 %s) TO STDOUT%s',
                    $self->{sqlprefix},
                    $SELECT,
                    $source_tablename,
                    ...
                    $srccmd = sprintf '%sCOPY (%s FROM ONLY %s t1 INNER JOIN public.settings t2 ON t1.id = t2.id WHERE t2.project_id=2 %s) TO STDOUT%s',
                    $self->{sqlprefix},
                    $SELECT,
                    $source_tablename,
                    $mode eq 'fullcopy' ? '' : " AND t1.$Table->{pklist} = ANY(?)",
